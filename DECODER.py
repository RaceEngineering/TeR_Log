...
class Signal:
    ...
    def _write_to_asc(self, df: pd.DataFrame, asc_file: str):
        """Guardar los datos en formato ASC (ASCII log de CAN compatible con Vector)."""
        with open(asc_file, 'w') as file:
            # Escribir el encabezado típico de un archivo ASC
            ...
            for _, row in df.iterrows():
                ...
                for signal_name, signal_value in row.items():
                    if signal_name == "Timestamp":
                        continue
                    # Aquí usamos un formato Vector estándar, incluyendo el ID y valor del mensaje
                    # Reemplaza 'ID' y 'NAME' con los valores apropiados para tu caso
...
if __name__ == "__main__":
    try:
        ...
        decoder.decode_log("RUN0.log", "test2_RUN0_prueba.asc", "asc", signals_to_plot=["rrRPM","rlRPM","APPS_AV","ANGLE"])  # Cambia Signal1, Signal2 por los nombres reales de las señales
    except Exception as e:
